...
for si, (spec_label, _) in enumerate(SPECIES):
    for ci, ctau_t in enumerate(CTAU_TARGETS):
        ...
        ax.set_xticklabels([f"{v}" for v in MPID_VALS])
        ax.set_yticks(range(n_mX))
        ax.set_yticklabels([f"{v}" for v in MX_VALS])
        ...
        ax.set_title(rf"$\pi^{{{spec_label}}}$,  $c\tau = {ctau_t}$ mm")
...
save_fig(fig1, "kappa_solutions_heatmap.pdf")
...
fig2.suptitle(r"Required $\kappa$ vs $m_X$  ($f_D = m_{\pi_D}$)")
...
for ci, ctau_t in enumerate(CTAU_TARGETS):
    ...
    ax.set_xticklabels([str(v) for v in MX_VALS], rotation=30)

axes2[0].set_ylabel(r"$\kappa$")
...
axes2[-1].legend(handles=color_handles + style_handles, loc="upper left", ncol=1)

fig2.tight_layout()
save_fig(fig2, "kappa_solutions_lines.pdf")
