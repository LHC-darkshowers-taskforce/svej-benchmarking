...
from dark_pion_widths import DarkPionModel, DIAGONAL_PION_INDICES
...
_DIAG_PDG = {2: 4900111, 7: 4900221, 14: 4900331}
...
_SPECIES_GETTERS = {
    ...
    "T8":    lambda m: m.ctau_diag_mm(7),
    "T3":    lambda m: m.ctau_diag_mm(2),
...
def solve_kappa(fD: float, mPiD: float, mXd: float, ctau_target: float,
    ...
    ref_model = DarkPionModel(fD=fD, m_piD=mPiD, m_X=mXd, kappa=1.0)
...
def write_param_card(path: str, mXd: float, mDarkQ: float, kappa: float) -> None:
...
def write_pythia_card(path: str, model: DarkPionModel, mXd: float,
    ...
    # --- lifetimes ---
    # Diagonal pions (T3=2, T8=7, T15=14): compute each separately
...
! --- Diagonal dark pion T3 (4900111), ctau = {ctau_diag[2]:.4f} mm ---
4900111:tau0 = {ctau_diag[2]:.6f}
{_diag_lines(2)}

! --- Diagonal dark pion T8 (4900221), ctau = {ctau_diag[7]:.4f} mm ---
4900221:tau0 = {ctau_diag[7]:.6f}
{_diag_lines(7)}
...
def print_summary(model: DarkPionModel, mXd: float, species: str,
...
def main():
    ...
    if args.kappa is not None and args.ctau is not None:
        ...
        try:
            kappa = solve_kappa(fD, args.mPiD, args.mXd, args.ctau, args.species)
    ...
    model = DarkPionModel(fD=fD, m_piD=args.mPiD, m_X=args.mXd, kappa=kappa)
    ...
    write_param_card(param_path, args.mXd, mDarkQ, kappa)
